    ...
    <div>
        ...
        <div class="wrap">
            <div class="search">
                ...
                <button type="submit" class="searchButton" onclick="like2()">
            ...
            <div>
                <table>
                    <tr>
                        ...
                        <td class="you"><c:if test="${d.status2=='未找到'}"><button type="button" onclick="select2('${d.tid}','${d.user}')"></c:if>
                            <c:if test="${d.status2=='已找到'}"><button type="button" disabled="disabled"></c:if>
                                ${d.status2}</button></td>
                    </tr>
                    <tr>
                        <td>品种：${d.kind}</td>
...
<script>
    function like2() {
        ...
        window.location.href='userSelectTieZi?like=like&index=寻宠&key='+key;
    }
    function select2(id,user) {
        localStorage.setItem("id",id);
        localStorage.setItem("user",user);
        console.log(id,user)
        window.location.href='Y_dianjixunchong.html';
    }
    function selectByType(type){
        window.location.href='userSelectTieZi?type='+type+'&index=寻宠';
    }
    function selectByKind(kind){
        window.location.href='userSelectTieZi?kind='+kind+'&index=寻宠';
    }
</script>
</body>
</html>
